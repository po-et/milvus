...
func (state *collectionState) putPrincipalTagsLocked(principalName string, entry *principalTagsEntry) {
	state.removePrincipalTagsLocked(principalName)
	if entry == nil {
		return
	}
	entry.cacheBytes = rlsutil.PrincipalTagsSize(principalName, entry.tags)
...
func (state *collectionState) trimPrincipalStateLocked() {
